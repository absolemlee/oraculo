...
import { convertLinesToCells } from '../../lib/hexagramUtils'
import { mapHexagramToIfaPair } from '../../lib/ifaMapper'
...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  // Convert to cell format for Ifá mapping
  const cells = convertLinesToCells(hex)
  const ifaPair = mapHexagramToIfaPair(cells)
  
  // Prepare changing lines
  ...
  // Simplified response focused on Ifá
  const response = {
    question,
    hexagram: {
      number,
      symbol: details.hex_font,
      english: details.english
    },
    ifa: {
      primary: ifaPair.primary.name,
      primaryGlyph: ifaPair.primary.glyph
    },
    changingLines: changingLineDetails.length > 0 ? changingLineDetails.map(line => ({
      line: line.line,
      text: line.text
    })) : null,
    transformed: hasChanging ? {
      hexagram: {
        number: transformedNumber,
        symbol: transformedDetails.hex_font,
        english: transformedDetails.english
      },
      ifa: {
        secondary: ifaPair.secondary.name,
        secondaryGlyph: ifaPair.secondary.glyph
      }
    } : null
  }
